...
public class Sql2oProductosRepositorio implements ProductosRepositorio {
    ...
    @Override public int crear(Producto producto) throws RepositorioException { 
        try (Connection conn = sql2o.open()) { 
    ...
    public Producto obtener(int id) throws RepositorioException {
        try (Connection conn = sql2o.open()) {
            ...
            return conn.createQuery(sql).addParameter("idProducto", id).throwOnMappingFailure(false).executeAndFetchFirst(Producto.class);
        } catch (Sql2oException e) {
            throw new RepositorioException();
        }
    }
